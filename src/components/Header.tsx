...
export default function Header({ title, sub }: { title: string; sub?: string }) {
  ...
  useEffect(() => {
    if (!session) return;
    setEmpresa(session.empresa_id || "SILOOPS");
  ...
  return (
    <header className="h-20 flex items-center justify-between px-8 sticky top-0 z-30 bg-[#07111f]/60 backdrop-blur-xl border-b border-[#1f334d]/50">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden xl:flex items-center gap-2 bg-[#101b2d]/50 px-3 py-2 rounded-2xl border border-[#1f334d]/50">
          <select value={empresa} onChange={(e) => setEmpresa(e.target.value)} className="bg-transparent text-[10px] text-white uppercase font-bold outline-none">
            <option value="SILOOPS">Empresa: SILOOPS</option>
          </select>
          <select value={usina} onChange={(e) => setUsina(e.target.value)} className="bg-transparent text-[10px] text-white uppercase font-bold outline-none">
            <option value="*">Usina: Todas</option>
            {session?.usinas.filter((item) => item !== "*").map((item) => (
              <option key={item} value={item}>{`Usina: ${item}`}</option>
            ))}
          </select>
          <select value={unidade} onChange={(e) => setUnidade(e.target.value)} className="bg-transparent text-[10px] text-white uppercase font-bold outline-none">
            <option value="*">Unidade: Todas</option>
            {session?.unidades.filter((item) => item !== "*").map((item) => (
              <option key={item} value={item}>{`Unidade: ${item}`}</option>
            ))}
          </select>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden md:flex items-center px-3 py-1.5 rounded-xl bg-[#101b2d]/60 border border-[#1f334d]/50 text-[10px] uppercase font-black tracking-widest text-[#8fdfff]">
            {session?.role || "SEM PERFIL"}
          </div>
