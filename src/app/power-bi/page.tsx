...
export default function PowerBiPage() {
  ...
  return (
    <>
      <Header title="Power BI" sub="Relatório executivo da operação SILO OPS" />
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <section className="lg:col-span-2 card overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#1e2d3d]">
              <div>
                <p className="text-[#c8d8e8] font-semibold">Incorporação do Power BI</p>
                <p className="text-[#4a6a8a] text-xs mt-0.5">Use um link público/seguro de incorporação do relatório.</p>
              </div>
            ...
            {embedUrl ? (
              <iframe
                title="Relatório Power BI SILO OPS"
                src={embedUrl}
              ...
              <div className="h-[360px] flex items-center justify-center text-center px-6">
                <div>
                  <p className="text-[#c8d8e8] font-semibold">Nenhum relatório Power BI configurado.</p>
                  <p className="text-[#4a6a8a] text-sm mt-1 max-w-xl">
                    Configure uma URL de incorporação. A integração oficial com token exige Azure AD e backend.
                  </p>
                </div>
              </div>
            )}
          ...
          <aside className="space-y-4">
            <form onSubmit={saveEmbed} className="card-p space-y-3">
              <p className="text-[#c8d8e8] font-semibold text-sm">Configuração</p>
              <input
            ...
            <div className="card-p space-y-3">
              ...
              <Row label="Bateria crítica" value={String(metrics.bateriaCritica)} danger={metrics.bateriaCritica > 0} />
              <Row label="Fonte da frota" value={s.equipErr ? "erro" : s.loading ? "carregando" : "online"} />
              <Row label="Fonte das operações" value={s.opsErr ? "erro" : s.loading ? "carregando" : "online"} />
            </div>
          </aside>
        </div>
      </main>
