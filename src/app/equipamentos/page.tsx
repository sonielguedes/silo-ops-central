...
export default function EquipamentosPage() {
  ...
  useEffect(() => {
    load();
    const id = setInterval(() => ref.current?.(), 10000);
    return () => clearInterval(id);
  }, [load]);

  const counts = equip.reduce((a, e) => {
    a[getDynamicPresence(e.last_seen)]++;
    return a;
        ...
        {loading ? (
          ...
          <div className="card overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#111820] border-b border-[#1e2d3d] text-[#4a6a8a] text-xs uppercase tracking-wider text-left">
                  <th className="px-4 py-3">Trator</th>
                  <th className="px-4 py-3">Presença</th>
                  <th className="px-4 py-3">Status Op.</th>
                  <th className="px-4 py-3">Último Sinal</th>
                  <th className="px-4 py-3">Bateria</th>
                  <th className="px-4 py-3">GPS</th>
                  <th className="px-4 py-3">Rastro</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1e2d3d]/50">
                {filtered.map((eq, idx) => {
                  const p = getDynamicPresence(eq.last_seen);
                  const active = selectedId === eq.trator_id;
                  return (
                    <tr
                      key={`${eq.trator_id}-${idx}`}
                      onClick={() => setSelectedId((prev) => (prev === eq.trator_id ? null : eq.trator_id))}
                      className={`hover:bg-[#1e2d3d]/30 cursor-pointer transition-colors border-l-2 ${active ? "border-l-[#00d4ff] bg-[#00d4ff]/5" : p === "ONLINE" ? "border-l-[#00e676]" : p === "INSTAVEL" ? "border-l-[#ffab00]" : "border-l-[#ff3d57]"}`}
                    >
                      ...
                      <td className="px-4 py-3"><Link href={`/equipamentos/${eq.trator_id}/rastro`} className="btn-ghost !py-2 !px-3 text-xs whitespace-nowrap">Ver Rastro</Link></td>
                    </tr>
                  );
                })}
              </tbody>
