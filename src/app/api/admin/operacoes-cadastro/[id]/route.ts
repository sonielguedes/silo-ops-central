...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const item = store.operacoes.find(it => it.id === params.id);
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const store = await readOperationRegistryStore();
  const existing = store.operacoes.find(it => it.id === params.id);
  ...
  try {
    const item = await upsertOperacao({
      ...body,
      id: params.id,
      empresa_id: existing.empresa_id, // keep original empresa_id unless admin global
    });
